...
\begin{abstract} %Hier kommt eine Zusammenfassung des Projektes
	...
	Das Matlab-Simulink Modell ist auf Github verfügbar. 
...
\begin{figure}[h] 
	...
	\caption{Darstellung des LKW mit aktiver Dämpfung}
	\label{DarstellungDerLadeflaeche}
\end{figure}

\section{Einleitung}
Wenn der Laderaum eines Lkws voll geladen ist und über eine Landstraße brettert, dann kann es doch schon mal etwas holprig werden. 
Das ist nicht nur für den Fahrer unangenehm. Es kann auch gefährlich für den LKW und seine Ladung werden.
Eine zu hart eingestellte Federung kann Unebenheiten nicht ausgleichen, so wird die Ladung und das Fahrwerk nicht geschützt. Wenn die 
Federung zu weich eingestellt ist kann sie die Last nicht halten. 

Wenn nur eine Feder eingesetzt wird kann es zu Schwingungen führen.
Um dies zu vermeiden wird eine Dämpfung verwendet. Hierbei wird zusätzlich zu einer Feder mit der Federkonstanten $K$ ein Dämpfer mit der Dämpfungskonstanten $\mu$ verwendet. 
...
auch ein pneumatischer Aktuator verwendet. Der Aktuator soll störungen entgegenwirken und den Ladebereich auf einer festgelegten Höhe halten.
...
Ein übermäßiges  Einschwingen ist zu vermeiden. Der LKW sollte nicht mit der Überdachung vor der Entladerampe kollidieren, also ist auch ein Überschwinger zu vermeiden.
...
Vorallem muss aber sichergestellt werden, dass die Ladung, und damit die Ladefläche zu jedem Zeitpunkt so sanft wie möglich bewegt werden soll. 
...
$x$ Entspricht der Höhe des Laderaums. Die positive x-Richtung zeigt nach oben. Die Feder wirkt mit der Kraft $F_{F} = K \cdot x$ nach oben. Der Dämpfer wirkt mit der Kraft 
...
Für diese Formel muss angenommen werden, dass sich die Feder für alle realistischen Auslenkungen $\Delta x$ im linearen elastischen 
...
Um die Gleichung in Simulink nachzubauen, wird sie nach ihrer höchsten Ableitung $a = \ddot{x}$ augelöst. 
Somit erhält man (\ref{eq:DifferenzengleichungAufgeloest}).
...
An dieser Stelle wird darauf verzichtet die Übertragungsfunktion des Systems zu definieren.
...
Mithilfe dieser Werte und der Annahme, dass es sich um ein lineares PT2 Glied handelt, könnte man nun die 
Übertragungsfunktion (\ref{eq:Uebertragungsfunktion}) im Laplace-Bereich bilden.
...
Aus der statischen Verstärkung $K$ ist zu erkennen, dass eine Große Kraft nötig ist, um die Ladefläche zu bewegen. 
Mit der Formel (\ref*{eq:statischeVerstaerkung}) kann ausgerechnet werden, welche Kraft nötig 
...
Es werden Zylinder ähnlich denen in \cite{Zylinder} zum Einsatz kommen.

In der Simulation wird zunächst keine Beschränkung für das Stellglied festgellegt.
...
Dieser Wert ist natürlich wichtig, um seinem Chef zu zeigen wie Komplex und schwer die Arbeit ist. Und dann natürlich auch, um vom Kunden einen 
höheren Preis zu verlangen zu können, nachdem man es dann doch geschafft hat dieses schwere Problem zu lösen. \smiley
...
Da es wie schon erwähnt in dieser Simulation keine Störungen gibt, wäre ein Anti-Aliasing-Filter nicht notwendig. Er sollte in diesem Fall sogar vermieden werden,
...
Es wird ein Sollwertsprung auf eine Mittlere Höhe von $x = \SI{0,5}{m}$ eingestellt. Bei $K_{p, Krit} = \SI{6,668e6}{} $ schwingt das 
System gleichmäßig mit konstanter Amplitude.
...
In Abbildung \ref{geregelteSprungantwortSchwingend} ist zu erkennen, dass das System recht schnell auf die Wunschhöhe einregelt.
...
Der Überschwinger ist minimal und der Wunschwert ist so gut wie sofort erreicht.
...
Mit einer Hydraulischen Kraft von $\SI{11,5}{kN}$ lässt sich nur eine Höhe von
...
Als Ziel der Regelung ist aber festgellegt, dass eine Höhe von $x = \SI{1}{m}$ erreicht werden soll. Die berechnete Kraft 
...
In jedem beliebigen Arbeitspunkt kann die Kraft der Feder berechnet werden, und auf die Stellgröße addiert werden.
...
das System aperiodisch verhält. Die Folgenden Parameter (\ref{eq:finaleParameter}) ergeben ein sehr gutes 
...
Dies ist ein annähernd optimales Verhalten. Die Beschleunigung würde im realen System wahrscheilich
langsamer ansteigen, da das Hydraulik Aggregat den nötigen Druck erstmal erzeugen muss.


\section{Beobachtungen und andere Effekte}
Ich habe eine weitaus größer Abtastrate gewählt als mein Kommilitone Eric Kern. Dies hatte zur Folge, dass mein System 
ganz anders reagiert hat als das seine.
Bei dem Stabilitätsrandverfahren sind ganz andere Ergebnisse herausgekommen. Und ich konnte mein System um Einiges Schneller regeln.
...
Aber der Trick mit dem wanderndem Arbeitspunkt hätte, wenn dies eine benotete Arbeit wäre, möglicherweise schon ein paar 
Extrapunkte verdient.

Eine Weitere wichtige Erkenntnis ist es, das sowohl das Parametriereverfahren, als auch die Simulink Tune Funktionalität
für das Erweiterte Modell keine guten Parameter hervorgebracht haben. Diese Parameter mussten manuell eingestellt
werden.
der $K_b$ Wert der Back-calculation hat auch einen signifikanten Einfluss auf das System. Jedoch habe ich hierfür 
keine möglichkeit gefunden ihn zu berechnen. Dies liegt wohl auch daran, dass ich immer noch keine Ahnung hab was diese 
Technik überhaupt macht. Trotzdem hat sie recht gut funktioniert.
...
erfüllt. Ich denke diese Lösung ist auch umsetztbar. Die Beschleunigung kann durch ändern von $F_{s, max}$ im Stellglied
angepasst werden, ohne das Regelverhalten zu zerstören.

Das Regelverhalten könne gestört werden, wenn sich die Masse verändert, oder Federkonstante nicht genau genug bestimmt ist.
Als mögliche Lösung könnte mithilfe eines Druck-Sensors im Hydraulik System die gesamt-Kraft gemessen werden. 
Daraus könnte dann die momentane Masse des Systems erfasst werden. So könnte der Regler dynamisch auf die Masse reagieren.


Letztentlich ist noch zu sagen, dass der LKW Fahrer wahrscheinlich gar kein Bedienfeld haben will, auf dem er 
die Höhe der Ladefläche eingeben kann.
Er weiß ja gar nicht wie hoch die Laderampe genau ist.
Er will wohl eher zwei Knöpfte: Einen zum hochfahren und einen zum runterfahren.
